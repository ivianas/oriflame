let sec03 = document.querySelector("div.wrapper>div.section03>div.product");

let s03 = async () => {
    let res = await (await fetch("http://localhost:2000/section03")).json();

    res.forEach(elem => {
        sec03.innerHTML += `
            <div class="w-auto h-[401px] ">
            
                <div class=" w-full h-auto bg-[#F6F6F6] relative rounded-[5px] !mb-[8px] overflow-hidden  ">

                    <div class=" w-[63px] h-[31px] bg-[#9CDABC] rounded-[5px] flex justify-center items-center font-[f2] text-[#333333] absolute  top-[15px] left-[15px] z-[1] ">
                        ${elem.new}
                    </div>
                    <img class="imgsec03  transition-all duration-500 " src="${elem.img}">
    ...
    let secc03=document.querySelector("div.section03");
    let brw=secc03.querySelector("button.browsec03");
    ...
    let vie=secc03.querySelector("button.viesec03");
